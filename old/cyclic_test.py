...
def flat_dataset(start, n):
    return [
        mlabdata.MlabDataEntry(
            start + datetime.timedelta(hours=i), 1000, 10000, 100)
        for i in range(n)
    ]
...
def slightly_cyclic_noisy_dataset(start, n):
    return [
        mlabdata.MlabDataEntry(
            ...
            10000 + 10000 * sin(float(i + 8) / 24.0 * 2 * pi) + 10 *
            random.random(),
...
class TestCyclicProblemFinder(unittest.TestCase):
    ...
    def test_cyclic_data_has_many_problems(self):
        ...
        cyclic_data = {
            netaddr.IPNetwork('1.1.0.0/16'):
            cyclic_dataset(self.start_time, 2000)
        }
    ...
    def test_short_cyclic_data_has_many_problems(self):
        # Should have one report for each attribute
        short_cyclic_data = {
            netaddr.IPNetwork('5.1.0.0/16'):
            cyclic_dataset(self.start_time, 100)
        }
        self.assertEqual(len(cyclic.find_problems(short_cyclic_data)), 3)

    def test_partially_cyclic_has_one_problem(self):
        # Should have one report, because just one attribute has a 24 hour
        # pattern
        partially_cyclic_data = {
            netaddr.IPNetwork('1.2.0.0/16'):
            slightly_cyclic_noisy_dataset(self.start_time, 2000)
        }
