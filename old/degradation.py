...
def _recursive_performance_degradation(netblock, timeseries, metric,
    ...
    recurse on indices into the original. This reduces our memory usage, but
    more importantly it removes the implicit requirement that each subsequent
    time period divide cleanly into the previous one.
    ...
    while period_index * period_size < end - start:
        ...
        if chunk_duration.days > 1:
            ...
            if _bad_slope(slope, metric):
                yield PerformanceDegradationProblem(netblock, chunk_duration,
                                                    metric_name)
            else:
                for problem in _recursive_performance_degradation(
                        netblock, timeseries, metric, metric_name, time_periods,
...
def _performance_degradation(netblock, timeseries, duration, metric,
    ...
    We look for these on three timescales: yearly, monthly, and weekly. This
    code returns PerformanceDegradationProblems, but it doesn't do needless
    work. In particular, if it finds a problem on a large timescale, then it
    will not look for the same problems on a smaller timescale.
    ...
    if _bad_slope(slope, metric):
        ...
        for problem in _recursive_performance_degradation(
                ...
                [hours_per_week, hours_per_month, hours_per_year], 0,
                len(timeseries)):
            yield problem
    ...
    for netblock, series in timeseries.iteritems():
        ...
        for metric, metric_name in [('upload_speed', 'Upload speeds'),
            ...
            problems_found.extend(
                list(
                    _performance_degradation(netblock, single_metric_series,
                                             duration, metric, metric_name)))
...
class PerformanceDegradationProblem(report.Problem):
    ...
    def __init__(self, netblock, duration, metric):
        if duration >= datetime.timedelta(days=364):
            ...
            priority = 5
        message = (
            '{metric} saw degraded performance from its historical values. '
