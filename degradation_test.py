...
class TestDegradationProblemFinder(unittest.TestCase):
    ...
    def test_find_slope(self):
        data = [i + 100 for i in range(730)]
        self.assertAlmostEqual(degradation._find_slope(data), 1)

    def test_degradation_finds_bad_last_week(self):
        # A good year
        performance = [i for i in range(365 * 24)]
        # But the slope is negative for the last week
        hours_per_week = 24 * 7
        performance[len(performance) - hours_per_week:] = reversed(
            performance[len(performance) - hours_per_week:])
        problems = list(
    ...
    def test_degradation_finds_bad_last_month(self):
        # A good year
        performance = [i for i in range(365 * 24)]
    ...
    def test_degradation_finds_bad_last_week_of_latency(self):
        # A good year
    ...
    def test_degradation_finds_whole_sample_problems(self):
        # 2 bad years
        ...
        self.assertEqual(len(problems), 1)
        self.assertEqual(
            problems[0]._duration, datetime.timedelta(days=2 * 365))
    ...
    def test_find_problems_finds_all_problems(self):
        ...
        hours_per_week = 7 * 24
        download[-hours_per_month:] = list(
            reversed(download[-hours_per_month:]))
        latency[-hours_per_month:-hours_per_month + hours_per_week] = list(
            reversed(
                latency[-hours_per_month:-hours_per_month + hours_per_week]))
        start = datetime.datetime(2016, 1, 1, 0, 0)
        data = [
            mlabdata.MlabDataEntry(
        ...
        problems = list(
            degradation.find_problems({
                netaddr.IPNetwork('10.0.0.0/8'): data
            }))
