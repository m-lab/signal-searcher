    ...
    try:
        while True:
            try:
                ...
                logging.warning('Could no consume_next: %s, assuming error was transient and reconnecting.', err)
            ...
            for key in sorted(response.rows):
                ...
                if 'data:download_speed_mbps_median' in row: 
                    ...
                    logging.debug('no download data for %s', key)
                    continue
                if 'data:upload_speed_mbps_median' in row: 
                    upload = row['data:upload_speed_mbps_median'][0].value
                    upload = struct.unpack('>d', upload)[0]
                else:
                    logging.debug('no upload data for %s', key)
                    continue
                if 'data:rtt_avg' in row:
                    rtt = row['data:rtt_avg'][0].value
                    rtt = struct.unpack('>d', rtt)[0]
                else:
                    logging.debug('no rtt data for %s', key)
                    continue
                if 'data:count' in row:
                    samples = int(row['data:count'][0].value)
                else:
                    logging.debug('no sample count data for %s', key)
                    continue
                data.append(
                    InternetData(key=key_part, table=table_name, time=date,
                        download=download, upload=upload, rtt=rtt,
                        samples=samples))
            response.rows.clear()
...
def problem_to_url(problem):
    ...
    return ('http://viz.measurementlab.net/compare/%s?' % comparison) + ('&'.join(str(k) + '=' + str(v) for (k,v) in args.items()))
...
def find_slope_problems(single_metric_series, metric):
    for i in range(len(single_metric_series) - 365):
         ...
         if _bad_slope(slope, metric):
             ...
             severity = percent_change_to_severity(expected_percent_loss,
                                                   metric)
             if severity == SEVERE:
...
def analyze_stream(key, data, method=find_slope_problems):
    if not data: return
    problems_found = []
    for metric, metric_name in [('download', 'Download speeds'),
    ...
    for problem_list in coalesced_problems:
        ...
        severity = sum(x.severity for x in problem_list) / float(len(problem_list))
        actual_problems.append(
                Problem(key.strip(), data[start].table, data[start].time, data[end].time,
                        problem_list[0].metric, severity, count))
    actual_problems = list(
            sorted(actual_problems, key=lambda x: x.severity * x.test_count))
