...
class QueryConditionalsTest(NormalizedStringTestCase):
    ...
    def setUp(self):
        ...
        self.conditional = query.QueryConditionals(start_time, end_time,
                                                   client_ip_block)

        self.generate_expected_nonmetric_dict(start_time, end_time,
                                              client_ip_block)
    ...
    def generate_expected_nonmetric_dict(self, start_time, end_time,
                                         client_ip_block):
        ...
        expected_client_ip_block = (
            ...
            'PARSE_IP(\'{start_addr}\') AND PARSE_IP(\'{end_addr}\')'.format(
                start_addr=str(ip_range[0]), end_addr=str(ip_range[1])))

        expected_complete_tcp = ('(web100_log_entry.snap.State = 1 '
    ...
    def test_download_dictionary_has_expected_metric_conditions(self):
        ...
        self.assertNormalizedStringsEqual(cond_dict['download'],
                                          expected_download)
        self.assertNormalizedStringsEqual(cond_dict['data_direction'],
    ...
    def test_upload_dictionary_has_expected_conditions(self):
        ...
        self.assertListEqual(actual_keys, [
            'client_ip_block', 'complete_tcp', 'data_direction', 'log_time',
            'upload'
        ])

        expected_upload = ('web100_log_entry.snap.HCThruOctetsReceived >= 8192 '
        ...
        self.assertEqual(cond_dict['client_ip_block'],
                         self.expected_nonmetric_dict['client_ip_block'])
        self.assertNormalizedStringsEqual(
            cond_dict['complete_tcp'],
            self.expected_nonmetric_dict['complete_tcp'])
        self.assertNormalizedStringsEqual(
            cond_dict['log_time'], self.expected_nonmetric_dict['log_time'])
    ...
    def test_minimum_rtt_dictionary_has_expected_metric_conditions(self):
        ...
        self.assertNormalizedStringsEqual(cond_dict['minimum_rtt'],
                                          expected_minimum_rtt)
        self.assertNormalizedStringsEqual(cond_dict['data_direction'],
                                          expected_data_direction)
...
class SubQueryGeneratorTest(NormalizedStringTestCase):
    ...
    def test_download_subquery_is_correct(self):
        subquery = query.SubQueryGenerator('download', self.start_time,
                                           self.end_time, self.client_ip_block)
        expected_query = (
            'SELECT '
    ...
    def test_upload_subquery_is_correct(self):
        subquery = query.SubQueryGenerator('upload', self.start_time,
                                           self.end_time, self.client_ip_block)
        expected_query = (
            'SELECT '
            'web100_log_entry.log_time AS timestamp, '
    ...
    def test_rtt_subquery_is_correct(self):
        subquery = query.SubQueryGenerator('minimum_rtt', self.start_time,
                                           self.end_time, self.client_ip_block)
        expected_query = (
            'SELECT '
            'web100_log_entry.log_time AS timestamp, '
    ...
    def test_subquery_with_mock_metric_raises_not_implemented_error(self):
        with self.assertRaises(NotImplementedError):
            query.SubQueryGenerator('mock_metric', self.start_time,
                                    self.end_time, self.client_ip_block)
...
class BuildMetricMedianQueryTest(NormalizedStringTestCase):
    ...
    def test_build_valid_download_median_query(self):
        actual = query.build_metric_median_query(
        ...
        expected = (
            ...
            '{subquery} '
            'GROUP BY date, hour '
            'ORDER BY date, hour').format(subquery=expected_download_subquery)
    ...
    def test_build_valid_upload_median_query(self):
        ...
        expected = (
            ...
            '{subquery} '
            'GROUP BY date, hour '
            'ORDER BY date, hour').format(subquery=expected_upload_subquery)
    ...
    def test_build_valid_rtt_median_query(self):
        ...
        expected = (
            ...
            '{subquery} '
            'GROUP BY date, hour '
            'ORDER BY date, hour').format(subquery=expected_rtt_subquery)
