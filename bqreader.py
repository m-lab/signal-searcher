...
def _get_bigquery_call_object(credentials_path, headless=False):
    ...
    auth_handler = bigquery_talk.GoogleAPIAuthHandler(credentials_path,
                                                      headless)
...
def _get_data(netblock, metric, start_time, end_time, credentials):
    query_string = query.build_metric_median_query(metric, start_time, end_time,
                                                   netblock)
...
def read_timeseries(netblocks, start_time, end_time, credentials):
  ...
  """
    ...
    for block in netblocks:
        data[block] = []
        _ = datetime.datetime(start_time.year, start_time.month, start_time.day,
                              start_time.hour, 0)

        upload_data = _get_data(block, 'upload', start_time, end_time,
                                credentials)
        download_data = _get_data(block, 'download', start_time, end_time,
                                  credentials)
        latency_data = _get_data(block, 'minimum_rtt', start_time, end_time,
                                 credentials)

        length = _fix_data_length(upload_data[1], download_data[1],
                                  latency_data[1])
